...
func (s *PersistedSQLStats) ScanEarliestAggregatedTs(
	...
	if earliestAggregatedTs.IsZero() {
		// if we are flushing every 15 seconds, it is unlikely that we have no persisted stats
		// but in this unlikely event, just return what the next aggregatedTs would be
