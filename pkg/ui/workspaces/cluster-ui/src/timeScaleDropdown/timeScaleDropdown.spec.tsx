...
describe("<TimeScaleDropdown> component", function() {
  ...
  it.only("allows selection of a custom time frame", () => {
    ...
    userEvent.click(getByDisplayValue(startText));
    userEvent.click(getByText("50"));
    userEvent.keyboard("{esc}");
    // userEvent.click(container.querySelector(".ant-dropdown")); // just clicking out to close the selection
    const newStartMoment = moment.utc({
      hour: 3,
      minute: 50,
    });
    getByDisplayValue(newStartMoment.format(customMenuTimeFormat));
