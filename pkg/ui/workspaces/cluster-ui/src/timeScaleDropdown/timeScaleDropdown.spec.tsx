...
import {
  timeFormat as customMenuTimeFormat,
  dateFormat as customMenuDateFormat,
} from "../dateRange";
...
describe("<TimeScaleDropdown> component", function() {
  ...
  it("allows selection of a custom time frame", () => {
    const mockSetTimeScale = jest.fn();
    // Default state
    ...
    // Custom menu should be initialized to currently selected time
    // Start: 3:28 AM (UTC)
    // End: 9:28 AM (UTC)
    ...
    getByDisplayValue(startText);
    getByDisplayValue(getNow().format(customMenuTimeFormat));

    // Further tests here testing changing the custom selection pending the 24h UTC change
  });
});
